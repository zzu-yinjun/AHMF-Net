...
def plot_specgram(batch_spec_tensor, label):
    ...
    for i in range(0, nb_display):
        spec = axes[i].imshow(
            batch_spec_tensor[i, ...].cpu().float().numpy(),
            origin="lower",
            cmap="magma",
        )
...
def plot_depth(batch_depth_tensor, label):
    ...
    for i in range(0, nb_display):
        depth = axes[i].imshow(
            batch_depth_tensor[i, 0, ...].cpu().float().numpy(), cmap="jet"
        )
        divider = make_axes_locatable(axes[i])
        cax = divider.append_axes("right", size="5%", pad=0.05)
...
def plot_input(batch_input_tensor, nb_display=4):
    ...
    for i in range(0, nb_display):
        for c in range(0, 2):
            spec = axes[c, i].imshow(
                batch_input_tensor[i, c, ...].data.cpu().float().numpy(),
                origin="lower",
                cmap="magma",
            )
...
def plot_pred_gt(batch_pred_tensor, batch_gt_tensor, nb_display=4):
    ...
    figure, axes = plt.subplots(2, nb_display, figsize=(16, 16))
    for i in range(0, nb_display):
        pred_depth = axes[0, i].imshow(
            batch_pred_tensor[i, 0, ...].data.cpu().float().numpy(), cmap="jet"
        )
        ...
        gt_depth = axes[1, i].imshow(
            batch_gt_tensor[i, 0, ...].data.cpu().float().numpy(), cmap="jet"
        )
